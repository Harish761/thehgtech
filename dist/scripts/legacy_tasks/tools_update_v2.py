    ...
    for root, dirs, files in os.walk('.'):
        ...
        for file in files:
            ...
            try:
                ...
                # Careful regex to remove old tools navigations without eating surrounding divs
                content = re.sub(r'[ \t]*<!--\s*Tools Navigation Link\s*-->[ \t]*\r?\n[ \t]*<a href="/tools/grc-assessment/"[^>]*>[\s\S]*?</a>[ \t]*\r?\n', '\n', content)
                
                # If there's an old Tools Dropdown
                content = re.sub(r'[ \t]*<!--\s*Tools Dropdown\s*-->[ \t]*\r?\n[ \t]*<div class="nav-dropdown">([\s\S]*?)</div>[ \t]*\r?\n[ \t]*</div>[ \t]*\r?\n', '\n', content)

                if '<!-- Tools Dropdown -->' not in content:
                    # insert fresh explicitly
